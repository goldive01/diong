...
describe("parsePageNumber", () => {
  it("defaults to 1 for anything invalid", () => {
    for (const bad of [null, undefined, "", "0", "-1", "1e2", "0x2", "abc", 0, -1, 1.5]) {
...
describe("getOffsetPagination", () => {
  ...
  it("falls back to page 1 for an invalid page", () => {
    ...
    expect(getOffsetPagination(-5).page).toBe(1);
  });
  ...
  it("respects a custom page size", () => {
    expect(getOffsetPagination(2, 10)).toEqual({ page: 2, pageSize: 10, offset: 10 });
  });
});
