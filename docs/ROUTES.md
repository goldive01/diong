...
| `/daily-prime` | Protected | Show today's assigned Prime Protocol. | Read protocol, complete Action Trigger, save/update today's reflection, open Prime history. | Protocol card skeleton. | No protocol assigned today. | Unable to load or complete protocol. |
| `/daily-prime/history` | Protected, onboarding required | List the owner's past Prime assignments, newest first, with a lightweight progress summary. | Open a historical Prime, open today's Prime. | Server page load. | "Your Prime journey starts with today's practice." | History unavailable falls back to an empty list; progress falls back to zeros. |
| `/daily-prime/history/[assignmentId]` | Protected, onboarding required | Read-only view of one past Prime assignment and the owner's saved reflection. | Read protocol and reflection; link to today's Prime when the assignment is today's. | Server page load. | Not applicable. | Invalid, missing or foreign assignment id returns not found. |
