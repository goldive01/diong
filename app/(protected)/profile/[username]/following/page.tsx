...
export default async function FollowingPage({
  ...
  const result = await listFollowing(supabase, social.id, page);
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-5 py-10 sm:px-6 sm:py-14">
      ...
      <h1 className="mt-4 text-3xl font-semibold tracking-tight">Following</h1>
      <p className="mt-1 font-medium text-[#657052]">@{social.username}</p>

      <div className="mt-8">
        <PersonList
          page={result}
          basePath={`/profile/${social.username}/following`}
