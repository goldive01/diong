// Pagination helpers for the communities application layer.
//
// A fresh, private copy — deliberately not reusing src/lib/social/pagination.ts
// — following that file's own precedent (restated by Pass 4's
// message-pagination.ts) of each pass owning its own cursor/offset logic so
// an earlier pass's semantics can never be disturbed by a later one.
...
export function encodeCommunityPostCursor(createdAt: string, id: number): string {
  return `${createdAt}|${id}`;
}

/**
 * Parse a keyset cursor value (used by list_community_posts). Returns null
 * for a missing, malformed or out-of-range cursor — the caller then serves
 * the first page rather than erroring.
 */
export function parseCommunityPostCursor(raw: unknown): Cursor | null {
