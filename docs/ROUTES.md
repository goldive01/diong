...
| `/feed` | Protected, onboarding required | Compose a post and read a chronological feed of your own posts plus public / followers-only posts from people you follow. | Write a post (type, body, visibility, 0–4 images); like / save; open a post; **Load more**. | "Your feed is quiet for now." | Feed falls back to an empty list on a read error. |
...
| `/posts/[id]/edit` | Protected, onboarding required, author only | Edit the body and visibility of your own post. | Update, save; remove an existing attached image (adding a new one during edit is deferred — see `docs/MEDIA_PROFILE_STORAGE.md`). | Not applicable. | Not visible or not your post → branded 404; field + form errors. |
...
| `/settings/profile` | Protected, onboarding required | Update the owner's public profile. | Update username, display name, bio; upload / replace / remove avatar and cover. | Existing profile values. | Field and form save errors. |
...
| `/communities/[slug]` | Protected, onboarding required | One community: description, rules, member count, owner, Join/Leave, and its post feed. | Join / Leave; share a post (members only); open Members / Moderation; **Report**; **Load more** posts; owner-only upload / replace / remove community avatar and cover. | "No posts yet." | Missing / inactive community → branded 404. |
...
`/journal/[id]/edit` are listed under V1 routes above. Profile / post /
community media ships in **Pass 7** (`docs/MEDIA_PROFILE_STORAGE.md`) — no
new routes; it adds upload/replace/remove controls to the existing
`/settings/profile`, `/posts/[id]/edit` and `/communities/[slug]` routes and
renders on every existing route that already shows an avatar or a post.
**Pass 7's migration has not yet been applied to any Supabase project** — see
that doc before relying on media behaving as described.
